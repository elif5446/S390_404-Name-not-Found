...
export class GoogleCalendarApi {
  ...
  private async fetchApi(endpoint: string, options: RequestInit = {}) {
    ...
    if (options.body) {
      try {
        ...
        console.log(
          `Request body:`,
          JSON.stringify(bodyObj, null, 2).substring(0, 200) + "...",
        );
      } catch (error) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          errorMessage =
            errorBody.error?.message || errorBody.error || errorMessage;
        } catch (parseError) {
        ...
        switch (response.status) {
          case 401:
            ...
            console.error(
              `Token used: ${this.accessToken.substring(0, 10)}...`,
            );
            break;
          case 403:
            console.error(
              `Permission denied. User may not have access to this calendar.`,
            );
            break;
          case 404:
          ...
          case 504:
            console.error(
              `Google API server error (${response.status}). This might be temporary.`,
            );
            break;
          default:
        ...
        throw new Error(
          `Google Calendar API Failure: ${errorMessage} (${response.status})`,
        );
      }
      ...
      if (
        ...
        console.error(
          `Network error: Unable to reach Google Calendar API. Check internet connection.`,
        );
  ...
  async getUpcomingEvents(maxResults: number = 10) {
    ...
    try {
      ...
      console.log(
        `Found ${result.items?.length || 0} upcoming events from primary calendar.`,
      );
      return result;
    } catch (error) {
  ...
  async getEventsFromCalendar(calendarId: string, maxResults: number = 10) {
    ...
    try {
      ...
      console.log(
        `Found ${result.items?.length || 0} events from calendar ${calendarId}.`,
      );
      return result;
    } catch (error) {
      console.error(
  ...
  async getEventsInRange(
    ...
    try {
      ...
      console.log(
        `Found ${result.items?.length || 0} events in date range from primary calendar.`,
      );
      return result;
    } catch (error) {
