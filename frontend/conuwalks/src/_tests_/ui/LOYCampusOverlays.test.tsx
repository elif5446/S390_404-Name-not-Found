...
describe("Loyola Campus Building Overlays", () => {
  ...
  const renderCampusPolygons = () => {
    return render(
      <CampusPolygons campus="LOY" geojson={geojson} metadata={metadata} />,
    );
  };
  ...
  describe("Building Colors", () => {
    it("should apply correct colors to all themed buildings", () => {
      ...
      buildingIds.forEach((buildingId) => {
        const expectedColor =
          BuildingTheme.LOY[buildingId as keyof typeof BuildingTheme.LOY] ??
          "#888888";
    ...
    it("all buildings should have accessibility labels matching their names", () => {
      ...
      buildingIds.forEach((buildingId) => {
        ...
        try {
          ...
          console.error(
            `Failed to find building: ${buildingId} - ${buildingName}`,
          );
          throw error;
        }
      });
