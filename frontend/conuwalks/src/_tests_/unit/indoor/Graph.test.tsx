...
const makeNode = (id: string, x: number, y: number, isEntrance = false): Node => ({
    id, floorId: 'floor-1', x, y, type: 'hallway', isEntrance,
});

const makeEdge = (nodeAId: string, nodeBId: string, accessible = true): Edge => ({
    nodeAId, nodeBId, accessible
});
...
describe("Graph", () => {
    ...
    describe("getEntranceNodes", () => {
        it("should return only nodes marked as entrances", () => {
            ...
            expect(entrances.map((n) => n.id)).toEqual(expect.arrayContaining(["A", "C"]));
        });
    ...
    describe("addEdge", () => {
        ...
        it("should create a bidirectional edge (A→B and B→A)", () => {
            graph.addEdge(makeEdge("A", "B"));
    ...
    describe("getNeighbors", () => {
        ...
        it("should return correct neighbors after adding edges", () => {
            ...
            expect(neighbors.map((n) => n.id)).toEqual(expect.arrayContaining(["B", "C"]));
        });
