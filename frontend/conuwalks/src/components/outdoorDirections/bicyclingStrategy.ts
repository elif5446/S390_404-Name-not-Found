...
export class BicyclingStrategy implements ITravelModeStrategy {
  ...
  applyTimeToRoutesBody(
    ...
    // Bicycling only supports departure time
  ...
  async fetchRoutes(
    ...
    try {
      ...
      const routes = mapRoutesApiResponse(rawRoutes, safeTargetTime, timeMode, "bicycling", this);
      ...
      if (error instanceof Error) {
        ...
        if (isRoutesBlockedError(error.message)) {
          const { rawRoutes, safeTargetTime } = await fetchLegacyApi(start, destination, this, targetTime, timeMode, "bicycling");
          const routes = mapLegacyApiResponse(rawRoutes, safeTargetTime, timeMode, "bicycling", this);
